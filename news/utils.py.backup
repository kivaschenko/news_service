...
def extract_links_from_news_site(base_url: str, max_links: int = 10) -> List[str]:
    """Extract article links from a news website's main page or sitemap"""
    try:
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36"
        }
...
def fetch_article_with_newspaper(url: str) -> Dict[str, str]:
    """Enhanced article extraction using newspaper3k library"""
    try:
        article = Article(url)
        article.download()
...
def fetch_article_with_bs4(url: str) -> Dict[str, str]:
    """Fallback article extraction using BeautifulSoup (improved version)"""
    try:
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36"
        }
...
def create_error_response(url: str, error_msg: str) -> Dict[str, str]:
    """Create standardized error response"""
    return {
        "title": f"Error fetching article from {urlparse(url).netloc}",
        "content": f"Failed to fetch content: {error_msg}",
